  ...
  <div id="vehicle-list" class="data-list-container">
    ...
    <vs-table ref="table" v-model="selected" pagination :max-items="itemsPerPage" search :data="usersData">
      ...
      <template slot="thead">
        ...
        <vs-th sort-key="name">SubTasks</vs-th>     
        <vs-th>Action</vs-th>
      </template>
        ...
        <vs-td>
        <p class="product-name font-medium truncate">{{ tr.subtask }}</p>
        </vs-td>
              <vs-td class="whitespace-no-wrap">
                <div class="demo-alignment">
                   <vs-button color="success" type="filled" icon-pack="feather" icon="icon-edit-2" @click.stop="manageRecord(tr.service_task_id)"></vs-button>
                   <vs-button color="warning" type="filled" icon-pack="feather" icon="icon-trash-2" @click.stop="deleteRecord(tr.service_task_id)"></vs-button>
                </div>
              </vs-td>
...
export default {
  ...
  methods: {
    addNewData: function()
    {
      this.$router.push('/services/task-create')
    },
    ...
    {
      var $this = this;
      axios.get(siteIndexURL+'/v1/servicetask')
      .then((result)=>{
         if (result.data.status == true) {
          this.usersData = result.data.response.ServiceTask;
        }
    ...
    manageRecord (id) {
      this.$router.push(`/services/task/edit/${id}`).catch(() => {})
    },
    deleteRecord (id) {
      ...
      var $this = this;
      axios.delete(siteIndexURL+'/v1/servicetask/'+id+'/')
      .then((result)=>{
