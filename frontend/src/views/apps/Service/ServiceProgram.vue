...
<template>
  <div id="vehicle-list" class="data-list-container">
    ...
    <vs-table ref="table" v-model="selected" pagination :max-items="itemsPerPage" search :data="usersData">
      ...
      <template slot="thead">
        <vs-th sort-key="Vehicle">Name</vs-th>
        <vs-th sort-key="Completion Date">Template</vs-th>
        <vs-th>Action</vs-th>
      </template>
          ...
          <tbody>
            <vs-tr :key="indextr" v-for="(tr, indextr) in data">
              ...
              <vs-td>
				...
				<vs-td>
				<p class="product-name font-medium truncate">{{ tr.template }}</p>
				</vs-td>

              <vs-td class="whitespace-no-wrap">
                <div class="demo-alignment">
                   <vs-button color="success" type="filled" icon-pack="feather" icon="icon-edit-2" @click.stop="manageRecord(tr.service_program_id)"></vs-button>
                   <vs-button color="warning" type="filled" icon-pack="feather" icon="icon-trash-2" @click.stop="deleteRecord(tr.service_program_id)"></vs-button>
                </div>
              </vs-td>
...
export default {
  ...
  methods: {
    addNewData: function()
    {
      this.$router.push('/services/program/create')
    },
	 ...
	 getProfile: function ()
    {
      ...
      .then((result)=>{
         if (result.data.status == true) {
          this.usersData = result.data.response.serviceprogram;
        }
    ...
    manageRecord (id) {
      this.$router.push(`/services/program/edit/${id}`).catch(() => {})
    },
    deleteRecord (id) {
      ...
      var $this = this;
      axios.delete(siteIndexURL+'/v1/serviceprogram/'+id)
      .then((result)=>{
  ...
  beforeMount() { 
    ...
    // this.usersData = this.definedStrings.vehicle.serviceProgram
  }
}
