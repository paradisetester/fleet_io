...
export default {
  ...
  data () {
   ...
   methods: {
    ...
    {
      var $this = this;
      axios.get(siteIndexURL+'/v1/vehiclemeterhistory/'+this.id+'/edit')
      .then((result)=>{
        if (result.data.status == true) {          
          var vehiclemeterhistory = result.data.response.vehiclemeterhistory;        
          this.vehicle_id = vehiclemeterhistory.vehicle_id
          this.date = vehiclemeterhistory.date         
          this.odometer_reading = vehiclemeterhistory.odometer_reading         
        }
