  ...
  <div id="vehicle-list" class="data-list-container">
    ...
    <vs-table ref="table" v-model="selected" pagination :max-items="itemsPerPage" search :data="usersData">
      ...
      <template slot="thead">
        ...
        <vs-th sort-key="Operator">Odometer Reading</vs-th>
        <vs-th>Action</vs-th>
      </template>
          ...
          <tbody>
            <vs-tr :key="indextr" v-for="(tr, indextr) in data">
              ...
              <vs-td>
				...
				<vs-td>
				<p class="product-name font-medium truncate">{{ tr.odometer_reading }} mi</p>
				</vs-td>
              ...
              <vs-td class="whitespace-no-wrap">
                <div class="demo-alignment">
                   <vs-button color="success" type="filled" icon-pack="feather" icon="icon-edit-2" @click.stop="manageRecord(tr.meter_id)"></vs-button>
                   <vs-button color="warning" type="filled" icon-pack="feather" icon="icon-trash-2" @click.stop="deleteRecord(tr.meter_id)"></vs-button>
                </div>
              </vs-td>
...
export default {
 ...
 computed: {
  ...
  methods: {
   addNewData: function()
    {
      this.$router.push('/meter/create')
    },
	 getProfile: function ()
    {
      var $this = this;
      axios.get(siteIndexURL+'/v1/vehiclemeterhistory')
      .then((result)=>{
         if (result.data.status == true) {
          this.usersData = result.data.response.vehiclemeterhistory;
        }
    ...
    manageRecord (id) {	
      this.$router.push(`/meter/edit/${id}`).catch(() => {})
    },
    deleteRecord (id) {
      ...
      var $this = this;
      axios.delete(siteIndexURL+'/v1/vehiclemeterhistory/'+id)
      .then((result)=>{
