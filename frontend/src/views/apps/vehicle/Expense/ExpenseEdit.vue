<template>
 ...
 <h6 class="mb-4">Expense Entry Details</h6>
	...
	<div class="mt-4">
      ...
      <select v-model="expense_type" class="w-full form-control">
        <option :value="expenseOptions.value" v-for='expenseOptions in expenseOptions' :key="expenseOptions.value" v-validate="'required'" name="expense_type">{{expenseOptions.label}}</option>
      </select>
    </div>
	...
	<div class="mt-4">
      ...
      <select v-model="frequency" class="w-full form-control">
        <option :value="frequencyOptions.value" v-for='frequencyOptions in frequencyOptions' :key="frequencyOptions.value" v-validate="'required'" name="frequency">{{frequencyOptions.label}}</option>
      </select>
    </div>
    </div>
	<div class="mt-4">
  ...
  <div class="vx-row box_outter mt-5">
            <template v-if="pageImage">
               ...
               <div class="modify-img flex justify-between mt-5">
                  ...
                  <vs-button type="flat" color="#999" @click="pageImage = ''; pageFile = ''">Remove File</vs-button>
               </div>
            </template>
         ...
         <div class="vx-row box_outter mt-5">
            <template v-if="pageDocument">
               ...
               <div class="modify-img flex justify-between mt-5">
                  ...
                  <vs-button type="flat" color="#999" @click="pageDocument = '';pageDocFile = ''">Remove File</vs-button>
               </div>
            </template>
...
export default {
  ...
  data () {
   ...
   methods: {
    ...
    {
      var $this = this;
      axios.get(siteIndexURL+'/v1/vehicleexpensehistory/'+this.id+'/edit')
      .then((result)=>{
        if (result.data.status == true) {          
          var VehicleExpenseHistory = result.data.response.VehicleExpenseHistory;

          this.expence_id = VehicleExpenseHistory.id
          this.vehicle_id = VehicleExpenseHistory.vehicle_id
          this.date = VehicleExpenseHistory.date
          this.time = VehicleExpenseHistory.time         
          this.price = VehicleExpenseHistory.price
          this.expense_type = VehicleExpenseHistory.expense_type
          this.vendor_id = VehicleExpenseHistory.vendor_id
          this.frequency = VehicleExpenseHistory.frequency         
          this.recurring = VehicleExpenseHistory.recurring         
          this.end_date = VehicleExpenseHistory.end_date         
          this.start_date = VehicleExpenseHistory.start_date   
          if (VehicleExpenseHistory.photo) {
            this.pageImage = this.siteIndexUrl+'/'+VehicleExpenseHistory.photo
            this.pageFile = VehicleExpenseHistory.photo  
          }      
          if (VehicleExpenseHistory.document) {
            this.pageDocument = this.siteIndexUrl+'/'+VehicleExpenseHistory.document
            this.pageDocFile = VehicleExpenseHistory.document
          }          
          this.comment = VehicleExpenseHistory.comment
          this.notes = VehicleExpenseHistory.notes
        }
    ...
    saveForm: function()
    {
      this.$validator.validateAll().then(result => {
        if (result) {
          ...
          formData.append("expense_type", $this.expense_type);
          ...
          formData.append("recurring", $this.recurring);
          formData.append("end_date", $this.end_date);
          formData.append("start_date", $this.start_date);
          formData.append("frequency", $this.frequency);
          formData.append("_method", 'PUT');
          axios.post(siteIndexURL+"/v1/vehicleexpensehistory/"+this.id, formData)
          .then( result => {
            if (result.data.status == true) {
