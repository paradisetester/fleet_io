...
<div class="mt-4">
  ...
  <div class="mt-4">
      <label class="text-sm">Labels</label>
        <select v-model="labels" v-validate="'required'" name="labels" class="w-full form-control">
        <option v-for="(pageStatusOptions, index) in pageStatusOptions" :key="index" :value="pageStatusOptions.value">{{pageStatusOptions.label}}</option>
        </select>
  </div>
  ...
  <div class="mt-4">
      <label class="text-sm">Country</label>
        <select v-model="country" v-validate="'required'" name="country" class="w-full form-control">
        <option v-for="(CountryOptions, index) in CountryOptions" :key="index" :value="CountryOptions.value">{{CountryOptions.label}}</option>
        </select>
  </div>
...
<div class="mt-4">
       <vs-checkbox v-model="fuel" vs-value="1" v-validate="'required'" name="fuel">Fuel</vs-checkbox>
    ...
    <div class="mt-4">
      <vs-checkbox v-model="service" vs-value="1" v-validate="'required'" name="service">Service</vs-checkbox>
    ...
    <div class="mt-4">
      <vs-checkbox v-model="vehicle" vs-value="1" v-validate="'required'" name="vehicle"> Vehicle</vs-checkbox>
...
export default {
  ...
  methods: {
    ...
    {
      var $this = this;
      axios.get(siteIndexURL+'/v1/vendor/'+this.vendor_id+'/edit')
      .then((result)=>{
        if (result.data.status == true) {          
          var vendor = result.data.response.vendor;

          this.vendor_id  = vendor.vendor_id
          this.name = vendor.name
          this.phone = vendor.phone
          this.website = vendor.website
          this.labels = vendor.labels
          ...
          this.fuel = vendor.fuel
          this.service = vendor.service
          this.vehicle = vendor.vehicle
        }
    ...
    saveForm: function()
    {
      this.$validator.validateAll().then(result => {
        if (result) {
          ...
          axios.post(siteIndexURL+"/v1/vendor/"+this.vendor_id, formData)
          .then( result => {    
            if (result.data.status == true) {
  ...
  beforeMount() {
    ...
    this.vendor_id = currentRoute.vendorid
    this.getPost();
