  ...
  <div id="vehicle-list" class="data-list-container">
    ...
    <vs-table ref="table" v-model="selected" pagination :max-items="itemsPerPage" search :data="usersData">
      ...
      <template slot="thead">
        ...
        <vs-th sort-key="name">Time</vs-th>
        <vs-th sort-key="name">Cost</vs-th>
        <vs-th sort-key="name">Odometer</vs-th>
        <vs-th sort-key="name">fuel_type</vs-th>      
        <vs-th>Action</vs-th>
      </template>
        ...
        <vs-td>
        <p class="product-name font-medium truncate">{{ tr.time }}</p>
        </vs-td>
        <vs-td>
        ...
        <vs-td>
        <p class="product-name font-medium truncate">{{ tr.fuel_type }}</p>
        </vs-td>
              <vs-td class="whitespace-no-wrap">
                <div class="demo-alignment">
                   <vs-button color="success" type="filled" icon-pack="feather" icon="icon-edit-2" @click.stop="manageRecord(tr.fuel_id)"></vs-button>
                   <vs-button color="warning" type="filled" icon-pack="feather" icon="icon-trash-2" @click.stop="deleteRecord(tr.fuel_id)"></vs-button>
                </div>
              </vs-td>
...
export default {
  ...
  methods: {
    ...
    {
      var $this = this;
      axios.get(siteIndexURL+'/v1/fuel')
      .then((result)=>{
         if (result.data.status == true) {
          this.usersData = result.data.response.fuel;
        }
    ...
    manageRecord (id) {
      this.$router.push(`/Fuel/edit/${id}`).catch(() => {})
    },
    deleteRecord (id) {
      ...
      var $this = this;
      axios.delete(siteIndexURL+'/v1/fuel/'+id)
      .then((result)=>{
