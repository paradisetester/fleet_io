...
<template>
  <div id="vehicle-list" class="data-list-container">
    ...
    <vs-table ref="table" v-model="selected" pagination :max-items="itemsPerPage" search :data="usersData">
      ...
      <template slot="thead">
        <vs-th sort-key="name">Make Name</vs-th>
...
export default {
  ...
  methods: {
    ...
    getProfile: function ()
    {
      ...
      .then((result)=>{
         if (result.data.status == true) {
          this.usersData = result.data.response.vehiclemake;
        }
    ...
    manageRecord (id) {
      this.$router.push(`/make/edit/${id}`).catch(() => {})
    },
    deleteRecord (id) {
      ...
      var $this = this;
      axios.delete(siteIndexURL+'/v1/vehiclemake/'+id)
      .then((result)=>{
