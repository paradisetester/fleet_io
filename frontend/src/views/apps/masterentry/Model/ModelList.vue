...
<template>
  <div id="vehicle-list" class="data-list-container">
    ...
    <vs-table ref="table" v-model="selected" pagination :max-items="itemsPerPage" search :data="usersData">
      ...
      <template slot="thead">
        <vs-th sort-key="name">Model Name</vs-th>
        <vs-th sort-key="name">Vehicle Make</vs-th>
        ...
        <template slot-scope="{data}">
          <tbody>
            <vs-tr :key="indextr" v-for="(tr, indextr) in data">
              ...
              <vs-td>
                <p class="product-name font-medium truncate">{{ tr.vehiclemake.name }}</p>
              </vs-td>
        <vs-td>
...
export default {
  ...
  methods: {
    ...
    getProfile: function ()
    {
      ...
      .then((result)=>{
         if (result.data.status == true) {
          this.usersData = result.data.response.vehiclemodel;
        }
    ...
    manageRecord (id) {
      this.$router.push(`/model/edit/${id}`).catch(() => {})
    },
    deleteRecord (id) {
      ...
      var $this = this;
      axios.delete(siteIndexURL+'/v1/vehiclemodel/'+id)
      .then((result)=>{
