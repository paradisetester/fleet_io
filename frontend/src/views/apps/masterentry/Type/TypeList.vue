...
<template>
  <div id="vehicle-list" class="data-list-container">
    ...
    <vs-table ref="table" v-model="selected" pagination :max-items="itemsPerPage" search :data="usersData">
      ...
      <template slot="thead">
        <vs-th sort-key="name">Type Name</vs-th>
        <vs-th sort-key="name">Vehicle Model</vs-th>
        ...
        <template slot-scope="{data}">
          <tbody>
            <vs-tr :key="indextr" v-for="(tr, indextr) in data">
              ...
              <vs-td>
                <p class="product-name font-medium truncate">{{ tr.vehicle_type__relation__with_model.name }}</p>
              </vs-td>
        <vs-td>
        <p class="product-name font-medium truncate" v-if="'1'===tr.status">Active</p>
        <p class="product-name font-medium truncate" v-else-if="'0'===tr.status">InActive</p>
        <p class="product-name font-medium truncate" v-else-if="'2'===tr.status">Pending</p>
        </vs-td>
...
export default {
  ...
  methods: {
    ...
    {
      var $this = this;
      axios.get(siteIndexURL+'/v1/vehicletype')
      .then((result)=>{
         if (result.data.status == true) {
          this.usersData = result.data.response.vehicletype;
        }
    ...
    manageRecord (id) {
      this.$router.push(`/type/edit/${id}`).catch(() => {})
    },
    deleteRecord (id) {
      ...
      var $this = this;
      axios.delete(siteIndexURL+'/v1/vehicletype/'+id)
      .then((result)=>{
