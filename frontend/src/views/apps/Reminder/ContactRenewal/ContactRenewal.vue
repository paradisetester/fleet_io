  ...
  <div id="vehicle-list" class="data-list-container">
    ...
    <vs-table ref="table" v-model="selected" pagination :max-items="itemsPerPage" search :data="usersData">
      ...
      <template slot="thead">
        ...
        <vs-th sort-key="name">Contact Type</vs-th>
        <vs-th sort-key="name">Due Date</vs-th>  
        <vs-th>Action</vs-th>
      </template>
        ...
        <template slot-scope="{data}">
          <tbody>
            <vs-tr :key="indextr" v-for="(tr, indextr) in data">
              ...
              <vs-td>
                <p class="product-name font-medium truncate">{{ tr.contact_type }}</p>
              </vs-td>
              <vs-td>
                <p class="product-name font-medium truncate">{{ tr.due_date }}</p>
              </vs-td>
...
export default {
  ...
  methods: {
    addNewData: function()
    {
      this.$router.push('/contact/renewal/create')
    },
    ...
    {
      var $this = this;
      axios.get(siteIndexURL+'/v1/contact_renewal_reminder')
      .then((result)=>{
         if (result.data.status == true) {
          this.usersData = result.data.response.ContactRenewal;
        }
    ...
    manageRecord (id) {
      this.$router.push(`/contact/renewal/edit/${id}`).catch(() => {})
    },
    deleteRecord (id) {
      ...
      var $this = this;
      axios.delete(siteIndexURL+'/v1/contact_renewal_reminder/'+id)
      .then((result)=>{
