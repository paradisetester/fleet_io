...
export default {
  ...
  methods: {
    ...
    {
      var $this = this;
      axios.get(siteIndexURL+'/v1/contact_renewal_reminder/'+this.id+'/edit')
      .then((result)=>{
        if (result.data.status == true) {          
          var fuel = result.data.response.ContactRenewal;

          this.id = fuel.id
          this.contact_name = fuel.contact_name
          this.contact_type = fuel.contact_type
          this.due_date = fuel.due_date
        }
    ...
    saveForm: function()
    {
      this.$validator.validateAll().then(result => {
        if (result) {
          ...
          axios.post(siteIndexURL+"/v1/contact_renewal_reminder/"+this.id, formData)
       .then( result => {
         if (result.data.status == true) {
