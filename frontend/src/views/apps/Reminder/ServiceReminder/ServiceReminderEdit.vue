<template>
  <vx-card no-shadow>
 <h6 class="mb-4">Service Reminder Details</h6>
...
<div class="mt-2">
      ...
      <v-select v-model="renewal_type.value" :options="renewalOptions" v-validate="'required'" name="renewal_type" />
  </div>
...
export default {
 ...
 flatPickr,  
  ...
  getPost: function ()
    {
      var $this = this;
      axios.get(siteIndexURL+'/v1/service_reminder/'+this.id+'/edit')
      .then((result)=>{
        if (result.data.status == true) {          
          var service_reminder = result.data.response.ServiceReminder;
          this.vehicle_id = service_reminder.name
          this.date = service_reminder.date         
          this.user_ids = service_reminder.user_ids         
          this.time_threshold = service_reminder.time_threshold         
          this.renewal_type.value = service_reminder.renewal_type         
          this.week = service_reminder.week         
        }
    ...
    saveForm: function()
    {
      this.$validator.validateAll().then(result => {
        if (result) {
          ...
          axios.post(siteIndexURL+"/v1/service_reminder/"+this.id, formData)
          .then( result => {
            if (result.data.status == true) {
              this.responseHandle('success', result.data.message);
              this.$router.push('/service/reminders')
            }
          }).catch(function (error) {
