...
export default {
 ...
 flatPickr,  
  ...
  getPost: function ()
    {
      var $this = this;
      axios.get(siteIndexURL+'/v1/vehicle_renewal_reminder/'+this.id+'/edit')
      .then((result)=>{
        if (result.data.status == true) {          
          var service_reminder = result.data.response.VehicleReminder;
          ...
          this.renewal_type = service_reminder.renewal_type         
    ...
    saveForm: function()
    {
      this.$validator.validateAll().then(result => {
        if (result) {
          ...
          axios.post(siteIndexURL+"/v1/vehicle_renewal_reminder/"+this.id, formData)
          .then( result => {
            if (result.data.status == true) {
