    ...
    <vs-table ref="table" v-model="selected" pagination :max-items="itemsPerPage" search :data="usersData">
      ...
      <template slot="thead">
        ...
        <vs-th sort-key="email">Email</vs-th>
         ...
         <vs-th sort-key="phone">Message</vs-th>     
        <vs-th>Action</vs-th>
      </template>
        ...
        <template slot-scope="{data}">
          <tbody>
            <vs-tr :key="indextr" v-for="(tr, indextr) in data">
              ...
              <vs-td>
                <p class="product-name font-medium truncate">{{ tr.phoneno }}</p>
              </vs-td>
              <vs-td>
...
export default {
  ...
  methods: {
    ...
    {
      var $this = this;
      axios.get(siteIndexURL+'/v1/contactus')
      .then((result)=>{
         if (result.data.status == true) {
          this.usersData = result.data.response.contactus;    
        }
    ...
    deleteRecord (id) {
      ...
      var $this = this;
      axios.delete(siteIndexURL+'/v1/contactus/'+id)
      .then((result)=>{
