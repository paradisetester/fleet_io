...
export default {
  ...
  methods: {
    saveForm: function()
    {
      this.$validator.validateAll().then(result => {
        if (result) {
          ...
          axios.post(siteIndexURL+"/v1/testimonials/"+this.post_id, formData)
          .then( result => {
    ...
    {
      var $this = this;
      axios.get(siteIndexURL+'/v1/testimonials/'+this.post_id+'/edit')
      .then((result)=>{
        if (result.data.status == true) {
          var post = result.data.response.post;
          this.post_id = post.post_id
          this.pageImage = this.siteIndexUrl+'/'+post.image
           this.pageFile = this.siteIndexUrl+'/'+post.image
  ...
  beforeMount() {
    ...
    this.post_id = currentRoute.postID
    this.getPost();
