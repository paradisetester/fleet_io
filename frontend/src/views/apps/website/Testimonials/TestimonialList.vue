  ...
  <div id="vehicle-list" class="data-list-container">
    ...
    <vs-table ref="table" v-model="selected" pagination :max-items="itemsPerPage" search :data="usersData">
      ...
      <template slot="thead">
        ...
        <vs-th sort-key="name">Author Name</vs-th>            
        ...
        <template slot-scope="{data}">
          <tbody>
            <vs-tr :key="indextr" v-for="(tr, indextr) in data">
              <vs-td>
                <img :src="siteIndexUrl+tr.image" width="50px" v-if="tr.image">
              ...
              <vs-td class="whitespace-no-wrap">
                <div class="demo-alignment">
                   <vs-button color="success" type="filled" icon-pack="feather" icon="icon-edit-2" @click.stop="manageRecord(tr.post_id)"></vs-button>
                   <vs-button color="warning" type="filled" icon-pack="feather" icon="icon-trash-2" @click.stop="deleteRecord(tr.post_id)"></vs-button>
                </div>
              </vs-td>
...
export default {
  ...
  methods: {
    ...
    getProfile: function ()
    {
      ...
      .then((result)=>{
         if (result.data.status == true) {
          this.usersData = result.data.response.posts;    
        }
    ...
    manageRecord (id) {
      this.$router.push(`/testimonials/edit/${id}`).catch(() => {})
    },
    deleteRecord (id) {
      ...
      var $this = this;
      axios.delete(siteIndexURL+'/v1/testimonials/'+id)
      .then((result)=>{
