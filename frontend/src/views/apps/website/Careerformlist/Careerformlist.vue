<!-- =========================================================================================
  File Name: CareerList.vue
  Description: Career List page
  ----------------------------------------------------------------------------------------
      ...
      <template slot="thead">
        ...
        <vs-th sort-key="email">Email</vs-th>
         <vs-th sort-key="phone">Phone</vs-th>     
          <vs-th sort-key="phone">Apply For</vs-th>      
        <vs-th>Action</vs-th>
      </template>
        ...
        <template slot-scope="{data}">
          <tbody>
            <vs-tr :key="indextr" v-for="(tr, indextr) in data">
              ...
              <vs-td>
                <p class="product-name font-medium truncate">{{ tr.apply_for }}</p>
              </vs-td>
...
export default {
  ...
  methods: {
    ...
    {
      var $this = this;
      axios.get(siteIndexURL+'/v1/career')
      .then((result)=>{
         if (result.data.status == true) {
          this.usersData = result.data.response.career;    
        }
    ...
    manageRecord (id) {
      this.$router.push(`/careerformlist/edit/${id}`).catch(() => {})
    },
    deleteRecord (id) {
      ...
      var $this = this;
      axios.delete(siteIndexURL+'/v1/career/'+id+'/')
      .then((result)=>{
