<template>
 ...
 <form v-on:submit.prevent>
  ...
  <span class="text-danger text-sm" v-show="errors.has('reported_date')">{{ errors.first('reported_date') }}</span>
    ...
    <vs-input class="w-full mb-4" v-model="summery" v-validate="'required'" name="summery"></vs-input>
    </div>
  ...
  <div class="mt-4">
      ...
      <v-select v-model="reported_by" :options="usersOptions"  v-validate="'required'" name="reported_by"/>
  </div>
  ...
  <div class="mt-4">
      ...
      <v-select v-model="assigned_to" :options="usersOptions"  v-validate="'required'" name="assigned_to"/>
  </div>
    ...
    saveForm: function()
    {
      this.$validator.validateAll().then(result => {
        if (result) {
          ...
          formData.append("assigned_to", $this.assigned_to.value);
          formData.append("pageFile", $this.pageFile);
          formData.append("pageDocFile", $this.pageDocFile);
          formData.append("reported_by", $this.reported_by.value);
          axios.post(siteIndexURL+"/v1/issue", formData)
          .then( result => {
