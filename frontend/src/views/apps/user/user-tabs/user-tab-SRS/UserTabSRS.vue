                                       ...
                                       <tbody>
                                          <tr class="tr-values vs-table--tr tr-table-state-null" v-for="(tabsTableData, keyIndex) in tabsTableDatas[keyInner]" :key="keyIndex">
                                             <td class="td vs-table--td" v-for="(field, fieldKey) in userTabFields.formFields" :key="fieldKey" v-if="field.tab[keyInner] == true && field.show_in.table == true">
                                                ...
                                                <span v-else-if="field.type == 'select'">{{definedStrings.srs[field.key][tabsTableData[field.key]]}}</span>
                                                <span v-else-if="field.key == 'amount_uninvested' || field.key == 'withdrawn_amount'">{{tabsTableData[field.key] | PRICEFORMAT}}</span>
...
import UserTabFields from './UserTabSRS'
...
export default {
	...
	methods: {
      ...
      openEditModalPopup: function (keyInner, tab, row) {
		...
		saveForm: function () {
			this.$validator.validateAll().then(result => {
               ...
               if (this.activeyKeyFormData.year_for_first_contribution != undefined && this.activeyKeyFormData.year_for_first_contribution != '') {
                  this.activeyKeyFormData.year_for_first_contribution = this.formatDate2(this.activeyKeyFormData.year_for_first_contribution);
               }
               ...
               if (edit_id == undefined) {
                  ...
                  axios.put(siteIndexURL+'/v1/srs/srs/edit/'+edit_id+'/', dataString)
                  .then((result) => {
                     $this.responseHandle('success', result.data.message);
                     this.popupActive = false;
                  }).catch(function (error) {
      ...
      deleteRecord: function(id)
      {
         ...
         var $this = this;
         axios.delete(siteIndexURL+'/v1/srs/srs/delete/'+id+'/')
         .then((result) => {
            $this.responseHandle('success', result.data.message);
      ...
      getInsuranceData: function()
      {
         ...
         axios.get(siteIndexURL+'/v1/srs/'+client_id+'/srs/')
            .then((result) => {
