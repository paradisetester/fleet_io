<template>
   <div class="vx-row box_outter">
            ...
            <vs-collapse accordion>
               <vs-collapse-item> 
                  <div class="tab_outter" slot="header">
                     <span class="user_hd">Term</span><span class="user_premium">Total Premium Amount : {{sumArray(tabTotalPrice) | PRICEFORMAT}}</span>
                  </div>
                              ...
                              <table class="vs-table vs-table--tbody-table">
                                 ...
                                 <tbody>
                                    <tr class="tr-values vs-table--tr tr-table-state-null" v-for="(tabsTableData, keyIndex) in tabsTableDatas" :key="keyIndex">
                                       ...
                                       <td class="td vs-table--td">{{tabsTableData.date_of_renewal | YYYYMMDD}}</td>
      ...
      <vs-popup fullscreen class="holamundo" :title="'Term'" :active.sync="popupActive">
            ...
            <div class="vx-row">
               ...
               <div class="vx-col sm:w-1/3 w-full" >
                  <div class="vx-row box_outter">
                     <vs-input
                        ...
                        data-vv-as="Premium amount by medisave"
                        v-model="activeyKeyFormData.premium_amount_by_medisave"
                        name="premium_amount_by_medisave"
                        label="Premium amount by medisave"
                        class="w-full mt-5"/>
                     <span class="text-danger text-sm" v-show="errors.has('premium_amount_by_medisave')">{{ errors.first('premium_amount_by_medisave') }}</span>
                  </div>
               </div>
               <div class="vx-col sm:w-1/3 w-full" >
                  <div class="vx-row box_outter">
               ...
               <div class="vx-col sm:w-1/3 w-full" >
                  <div class="vx-row box_outter">
                     <vs-input
                        icon-pack="feather" icon="icon-dollar-sign" icon-before icon-no-border
                        data-vv-as="Total premium if throughout tenure"
                        readonly
                        v-model="total_premium_if_throughout_tenure"
                        name="total_premium_if_throughout_tenure"
                        label="Total premium if throughout tenure"
                        class="w-full mt-5"/>
                     <span class="text-danger text-sm" v-show="errors.has('total_premium_if_throughout_tenure')">{{ errors.first('total_premium_if_throughout_tenure') }}</span>
                  </div>
               </div>
               <div class="vx-col sm:w-1/3 w-full" >
                  <div class="vx-row box_outter">
...
export default {
   ...
   computed: {
      ...
      premium_paid_till_date: function(){
         ...
         if (this.activeyKeyFormData.mode_of_payment == 'MNT') {  
            ...
            paidMonth = this.activeyKeyFormData.total_tenure - this.getRemainingTenure(this.activeyKeyFormData.total_tenure, 'yearly');
         }
   ...
   methods: {
      ...
      {
         return (this.activeyKeyFormData.mode_of_payment == 'MNT'?((this.activeyKeyFormData.premium_amount_by_medisave*12)+(this.activeyKeyFormData.premium_amount_by_cash*12)):(this.activeyKeyFormData.mode_of_payment == 'ANL' || this.activeyKeyFormData.mode_of_payment == 'LPS'?(parseFloat(this.activeyKeyFormData.premium_amount_by_cash)+parseFloat(this.activeyKeyFormData.premium_amount_by_medisave)):0));
      },
      ...
      resetForm: function()
      {
         var formField = {
            ...
            policy_number:'',
            mode_of_payment:'',
            premium_amount_by_medisave:0,
            premium_amount_by_cash:0,
            date_in_force:'',
            date_of_renewal:'',
            total_tenure:0,
            tenure_remaining:0,
            accident_death:'',
            death:'',
            total_premium_if_throughout_tenure:'',
      ...
      saveForm: function () {
         this.$validator.validateAll().then(result => {
            if (result) {
               ...
               this.activeyKeyFormData.total_premium_if_throughout_tenure = this.total_premium_if_throughout_tenure
               var $this = this;
               ...
               if (result.data.success == true) {
                  ...
                  for (var i = 0; i < rows.length; i++) {
                     ...
                     if (row.insurance_type == this.insurance_type) {
                        var tabTotalPrice = (row.mode_of_payment == 'MNT'?((row.premium_amount_by_medisave*12)+(row.premium_amount_by_cash*12)):(row.mode_of_payment == 'ANL' || row.mode_of_payment == 'LPS'?(parseFloat(row.premium_amount_by_cash)+parseFloat(row.premium_amount_by_medisave)):0));                        
