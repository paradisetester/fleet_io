            ...
            <div class="vx-row box_outter">
              ...
              <div class="vx-col sm:w-1/1 w-full" v-if="item.addPhoto == true">
                 <div class="flex flex-wrap items-center mb-base">
					...
					<img :src="imagepath+'/'+item.itemRemarkPhoto"  width="200px"/>
              ...
              <div class="flex flex-wrap items-center mb-base">
                <vs-input v-model="item.itemMeterOdoMeter" class="w-full mb-base" label-placeholder="Odometer"></vs-input>
                 <div class="flex flex-wrap items-center mb-base">
                    ...
                    <vs-upload multiple text="Pick file or Drop file here" :action="uploadFile"  fileName='photos' automatic  @on-success="successUpload" @on-delete="successDelete($event,index)" :data="{'index':index}"  />
                 </div>
              </div>
              <div class="vx-col sm:w-1/1 w-full">
                 <a class="dropdown-toggle" v-on:click="item.addRemark = true" data-toggle="dropdown" aria-expanded="false">+ Add Remark</a>
...
export default {
  ...
  methods: {
    ...
    saveForm: function()
    {
      ...
      axios.put(siteIndexURL+"/v1/vehicleInspection/"+this.id, dataString)
      .then( result => {
          if (result.data.status == true) {   
    ...
    {
      var $this = this;
      axios.get(siteIndexURL+'/v1/vehicleInspection/'+this.id)
      .then((result)=>{
        if (result.data.status == true) {
          var vehicleInspection = result.data.response.vehicleInspection;
          this.items = JSON.parse(vehicleInspection.fields);
          this.vehicle_id = vehicleInspection.vehicle_id;
        }
      }).catch(function (error) {
        if (error.response) {
          $this.responseHandle('warning', error.response.data.message);
