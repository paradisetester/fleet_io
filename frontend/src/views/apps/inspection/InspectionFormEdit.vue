...
export default {
  ...
  methods: { 
    ...
    {
      var $this = this;
      axios.get(siteIndexURL+'/v1/inspectiontemplates/'+this.template_id+'/edit')
      .then((result)=>{       
        if (result.data.status == true) {  
          const inspectiontemplate = result.data.response.inspectiontemplate;
          this.description = inspectiontemplate.description
          this.title = inspectiontemplate.title
        }
    ...
    saveForm: function()
    {
      this.$validator.validateAll().then(result => {
        if (result) {
          ...
          axios.post(siteIndexURL+"/v1/inspectiontemplate/update/"+this.template_id, formData)
          .then( result => {
            if (result.data.status == true) {
