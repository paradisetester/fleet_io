  ...
  <div id="vehicle-list" class="data-list-container">
    ...
    <vs-table ref="table" v-model="selected" pagination :max-items="itemsPerPage" search :data="usersData">
      ...
      <template slot="thead">
        ...
        <vs-th sort-key="Contact Name">Contact Name</vs-th>
        <vs-th sort-key="Contact Email">Contact Email</vs-th>
        <vs-th sort-key="Updated At">Updated At</vs-th>
        <vs-th>Action</vs-th>
      </template>
          ...
          <tbody>
            <vs-tr :key="indextr" v-for="(tr, indextr) in data">
              ...
              <vs-td>
				...
				<vs-td>
				<p class="product-name font-medium truncate">{{ tr.website }}</p>
				</vs-td>
        <vs-td>
        <p class="product-name font-medium truncate">{{ tr.contact_name }}</p>
        </vs-td>
        <vs-td>
        <p class="product-name font-medium truncate">{{ tr.contact_email }}</p>
        </vs-td>
              <vs-td>
                <p class="product-name font-medium truncate">{{ tr.updated_at }}</p>
              </vs-td>
              <vs-td class="whitespace-no-wrap">
                <div class="demo-alignment">
              <vs-button color="success" type="filled" icon-pack="feather" icon="icon-edit-2" @click.stop="manageRecord(tr.vendor_id)"></vs-button>
              <vs-button color="warning" type="filled" icon-pack="feather" icon="icon-trash-2" @click.stop="deleteRecord(tr.vendor_id)"></vs-button>
                </div>
              </vs-td>
...
export default {
  ...
  methods: {
    ...
    getProfile: function ()
    {
      ...
      .then((result)=>{
         if (result.data.status == true) {
          this.usersData = result.data.response.vendors;
        }
    ...
    manageRecord (id) {
      this.$router.push(`/vendor/edit/${id}`).catch(() => {})
    },
    deleteRecord (id) {
      ...
      var $this = this;
      axios.delete(siteIndexURL+'/v1/vendor/'+id)
      .then((result)=>{
